...
From within the *green_engine_src* directory push your app to IBM Cloud
  ```
cf push
  ```
...
1. Create a file called `vcap-local.json` in the `green_engine_src` directory with the following content:
  ```
  {
    "services": {
      "cloudantNoSQLDB": [
...
cf push
  ...
  View app at the URL listed in the output of the push command, for example, *myUrl.mybluemix.net*.
